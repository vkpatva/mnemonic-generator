...
/// # Parameters
/// - `length`: Number of words in the mnemonic. Must be a multiple of 3 (12, 15, 18, 21, or 24).
///
/// # Returns
/// - A `Result` with the generated mnemonic as a space-separated string or an error message.

pub fn generate_mnemonic(length: usize) -> Result<String, String> {
    // Validate length
    if length % 3 != 0 || length < 12 || length > 24 {
        return Err("Invalid length. Must be 12, 15, 18, 21, or 24.".to_string());
    }
    // for 12 words, 128 bits of entropy are needed ie 16 bytes
    let entropy_bytes = length / 3 * 4;

    let entropy: Vec<u8> = (0..entropy_bytes)
        .map(|_| rand::thread_rng().gen())
        .collect();

    // Calculate checksum from the entropy
    let hash = sha2::Sha256::digest(&entropy);

    let checksum_bits = length / 3;
    let checksum = (hash[0] >> (8 - checksum_bits)) as u8;
    ...
    for i in 0..checksum_bits {
        bits.push((checksum >> (checksum_bits - 1 - i)) & 1 == 1);
    }

    // Convert bits to indices
    let word_indices: Vec<usize> = bits
        .chunks(11)
        .map(|chunk| chunk.iter().fold(0, |acc, &b| (acc << 1) | b as usize))
        .collect();

    // Map indices to words using the
    let mnemonic = word_indices
        .iter()
...
mod tests {
    ...
    fn test_generate_mnemonic_invalid_length() {
        let result = generate_mnemonic(10);
        assert!(result.is_err());
        assert_eq!(
            result.unwrap_err(),
            "Invalid length. Must be 12, 15, 18, 21, or 24."
        );
    }
}
